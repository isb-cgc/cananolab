...
var app = angular.module('angularApp')
  .controller('EditSynthesisMaterialsCtrl', function (sampleService, utilsService, navigationService, groupService, $rootScope, $scope, $http, $location, $filter, $modal, $routeParams) {
    ...
    if ($scope.synMaterialId == -1) {
      ...
      $http({
        method: 'GET',
        url: `/caNanoLab/rest/synthesisMaterial/edit?sampleId=${$scope.sampleId}&synMaterialId=${$scope.synMaterialId}`
      }).
      then(function (data, status, headers, config) {
        data = data['data']
        $scope.material = data;
      ...
      then(function (data, status, headers, config) {
        ...
        for (var x = 0; x < $scope.fileArray.length; x++) {
          $http({
            ...
            data: [$scope.material, $scope.fileArray[x]]
          }).
          then(function () {
            data = data['data']
        ...
        $location.search({
          'message': 'Synthesis Material successfully saved.',
          'sampleId': $scope.sampleId
        }).path('/editSynthesis').replace();
      }).
      catch(function (data, status, headers, config) {
